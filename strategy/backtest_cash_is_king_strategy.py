...
# *** 核心簡化：策略報酬率現在只由 QQQ 部位決定 ***
strategy_returns = (positions.shift(1) * asset_returns[[ATTACK_ASSET]]).sum(axis=1).fillna(0)
...
# 上圖：權益曲線
strategy_equity.plot(ax=ax1, label='現金為王策略', color='gold')
...
# 標示防禦區間
is_defense = positions[ATTACK_ASSET] == 0
if is_defense.sum() > 0:
    ...
    for s, e in zip(start_dates, end_dates):
        ax1.axvspan(s, e, color='grey', alpha=0.2)
        ax2.axvspan(s, e, color='grey', alpha=0.2)

from matplotlib.patches import Patch
handles, labels = ax1.get_legend_handles_labels()
handles.append(Patch(color='grey', alpha=0.2))
labels.append('防禦模式 (持有現金)')
