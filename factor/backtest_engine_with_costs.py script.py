...
class ProfessionalBacktester:
    ...
    def run(self):
        ...
        # 計算每日的成本
        tx_cost_rate = self.config['costs']['transaction_pct']
        slippage_rate = self.config['costs']['slippage_pct']
        total_cost_rate = tx_cost_rate + slippage_rate
        ...
        # 我們用「扣除成本前」的報酬率來計算波動率和槓桿，這更符合現實
    ...
    def analyze_and_plot(self):
        ...
        # 計算毛、淨權益曲線
        strategy_equity_gross = (1 + self.strategy_returns).cumprod()
        strategy_equity_net = (1 + self.strategy_returns_net).cumprod()
        ...
        strategy_metrics_gross = calculate_metrics(self.strategy_returns)
        strategy_metrics_net = calculate_metrics(self.strategy_returns_net)
        benchmark_metrics = calculate_metrics(benchmark_returns)

        print("\n--- Backtest Results ---")
        print("\n[Strategy - GROSS Performance (Before Costs)]")
        for key, value in strategy_metrics_gross.items(): print(f"{key}: {value:.2%}" if key != "Sharpe Ratio" else f"{key}: {value:.2f}")
        
        print("\n[Strategy - NET Performance (After Costs)]")
        for key, value in strategy_metrics_net.items(): print(f"{key}: {value:.2%}" if key != "Sharpe Ratio" else f"{key}: {value:.2f}")
        ...
        strategy_equity_gross.plot(ax=ax, label='Strategy (Gross)', color='cyan', linestyle='--')
        strategy_equity_net.plot(ax=ax, label='Strategy (Net)', color='blue', linewidth=2)
        benchmark_equity.plot(ax=ax, label='Benchmark (Buy and Hold QQQ)', color='grey', linestyle=':')

        ax.set_title('Final Strategy Performance: Gross vs. Net of Costs')
        ax.set_xlabel('Date')
...
if __name__ == '__main__':
    engine = ProfessionalBacktester(config)
    engine.run()
