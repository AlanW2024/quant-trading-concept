...
# *** 全新「雙重確認」出場規則 ***
VIX_EXIT_THRESHOLD = 22  # 條件一：VIX 必須低於 22，代表市場情緒穩定
MA_WINDOW = 20         # 條件二：QQQ 價格必須站上 20 日均線，代表動能恢復
...
for i in range(1, len(prices_open)):
    ...
    if current_state == 'ATTACK':
        ...
        if all(v is not None for v in [vix_yesterday, qqq_close_yesterday, qqq_ma_yesterday]):
            ...
            # 條件二：價格動能是否恢復？
            is_momentum_ok = qqq_close_yesterday > qqq_ma_yesterday
            
            # *** 只有兩個條件「同時」滿足，才解除防禦 ***
            if is_sentiment_ok and is_momentum_ok:
                current_state = 'ATTACK'
...
# 上圖：權益曲線
strategy_equity.plot(ax=ax1, label='雙重確認出場策略', color='darkviolet')
