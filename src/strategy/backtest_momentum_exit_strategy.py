...
for i in range(1, len(prices_open)):
    ...
    if current_state == 'ATTACK':
        ...
        # 確保數據都存在
        if qqq_close_yesterday is not None and qqq_ma_yesterday is not None:
            # 條件：QQQ 的收盤價是否重新站上 20 日均線？
            if qqq_close_yesterday > qqq_ma_yesterday:
                # 解除防禦！切換回攻擊模式
                current_state = 'ATTACK'
...
# 上圖：權益曲線
strategy_equity.plot(ax=ax1, label='動量出場策略', color='crimson')
...
if is_defense.sum() > 0:
    is_start = (is_defense.diff() == 1)
    is_end = (is_defense.diff() == -1)
